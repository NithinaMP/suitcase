...
class _AppShellState extends State<AppShell> {
  late int _idx;
  final _pages  = const [HomeView(insideShell: true), TripSetupView(), SavedView()];
  final _labels = ['LOOKS', 'TRAVEL', 'SAVED'];
  final _syms   = ['◆', '✦', '○'];
  ...
  Widget build(BuildContext context) {
    ...
    if (isWeb) {
      return Scaffold(
        ...
        body: Column(children: [
          _TopNav(idx: _idx, labels: _labels, syms: _syms, auth: auth,
              onTab: (i) => setState(() => _idx = i), onSignOut: _signOut),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Material(
          color: Colors.transparent,
          child: _BottomNav(idx: _idx, labels: _labels, syms: _syms,
...
class _TopNav extends StatelessWidget {
  final int idx;
  final List<String> labels, syms;
  final ap.SuitcaseAuthProvider auth;
  final void Function(int) onTab;
  final VoidCallback onSignOut;

  const _TopNav({required this.idx, required this.labels, required this.syms,
    required this.auth, required this.onTab, required this.onSignOut});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(color: SColors.bg,
          border: Border(bottom: BorderSide(color: SColors.lightDivider))),
            ...
            child: Padding(
              ...
              child: Row(children: [
                Text('SUITCASE', style: GoogleFonts.cormorantGaramond(
                    ...
                    letterSpacing: 6, color: SColors.ink)),
                const SizedBox(width: 48),
                ...List.generate(labels.length, (i) {
                  final active = i == idx;
                  return GestureDetector(
                      ...
                      child: Container(
                          ...
                          decoration: BoxDecoration(
                              border: Border(bottom: BorderSide(
                                  color: active ? SColors.gold : Colors.transparent,
                                  width: 2))),
                          child: Text(labels[i], style: STextStyles.label(12,
                              color: active ? SColors.ink : SColors.warmGray,
                              letterSpacing: 2))));
                }),
                const Spacer(),
                _ProfileMenu(auth: auth, onSignOut: onSignOut),
              ]),
            ),
          ),
        ),
...
class _ProfileMenu extends StatefulWidget {
  final ap.SuitcaseAuthProvider auth;
  final VoidCallback onSignOut;
  const _ProfileMenu({required this.auth, required this.onSignOut});
  @override
            ...
            child: Material(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                    ...
                    borderRadius: SRadius.lg,
                    border: Border.all(color: SColors.lightDivider),
                    boxShadow: [BoxShadow(color: SColors.ink.withOpacity(0.1),
                ...
                child: Column(mainAxisSize: MainAxisSize.min,
                    ...
                    children: [
                              ...
                              if (widget.auth.userStyleVibe != null) ...[
                                ...
                                Container(
                                    ...
                                    decoration: BoxDecoration(
                                        color: SColors.goldLight,
                                        borderRadius: SRadius.full),
                                    child: Text(widget.auth.userStyleVibe!,
                                        style: STextStyles.caption(10,
                                            color: SColors.goldDark))),
                              ],
                            ]),
                      ...
                      Container(height: 1, color: SColors.lightDivider),
                      GestureDetector(
                          onTap: () { _close(); widget.onSignOut(); },
                          child: Padding(
                              padding: const EdgeInsets.all(16),
                              child: Row(children: [
                                Icon(Icons.logout_rounded, size: 16, color: SColors.error),
                                const SizedBox(width: 10),
                                Text('Sign out', style: STextStyles.label(13,
                                    color: SColors.error, letterSpacing: 0.3)),
                              ]))),
                    ]),
              ),
            ),
        ...
        child: GestureDetector(
            ...
            child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                    ...
                    borderRadius: SRadius.full),
                alignment: Alignment.center,
                child: Text(initial, style: STextStyles.label(14,
                    color: SColors.bg, letterSpacing: 0)))));
  }
}
...
class _BottomNav extends StatelessWidget {
  ...
  final List<String> labels, syms;
  final void Function(int) onTap;
  const _BottomNav({required this.idx, required this.labels,
    required this.syms, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    height: 60,
    decoration: BoxDecoration(
    color: SColors.ink, borderRadius: SRadius.xl,
    boxShadow: [BoxShadow(color: SColors.ink.withOpacity(0.2),
    ...
    Text(syms[i], style: TextStyle(fontSize: 13,
    color: active ? SColors.ink : SColors.warmGray)),
    const SizedBox(height: 2),
